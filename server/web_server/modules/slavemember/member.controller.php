...
class memberController extends member
{
	...
	function procMemberInsert()
	{
		...
		$roletest = unserialize($extra_vars);
		$roletest2 = array_slice($roletest,2);
	...
	function insertMember(&$args, $password_is_hashed = false)
	{
		...
		//if($args->homepage && !preg_match("/^[a-z]+:\/\//i",$args->homepage)) $args->homepage = ''.$args->homepage;
		//if($args->blog && !preg_match("/^[a-z]+:\/\//i",$args->blog)) $args->blog = ''.$args->blog;
		...
		//$customQuery = executeQuery('member.customQuery', $args);
		if(!$output->toBool())
		{
			$oDB->rollback();
			return $output;
	...
	function updateMember($args, $is_admin = FALSE)
	{
		...
		$args->homepage = htmlspecialchars($args->homepage, ENT_COMPAT | ENT_HTML401, 'UTF-8', false);
		$args->blog = htmlspecialchars($args->blog, ENT_COMPAT | ENT_HTML401, 'UTF-8', false);
		if($args->homepage && !preg_match("/^[a-z]+:\/\//is",$args->homepage)) $args->homepage = ''.$args->homepage;
		if($args->blog && !preg_match("/^[a-z]+:\/\//is",$args->blog)) $args->blog = ''.$args->blog;
