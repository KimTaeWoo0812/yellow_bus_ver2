...
if($__Context->filter_type==''){ ?>(<?php echo $__Context->total_count ?>)<?php } ?></a>
			<div class="x_pull-right x_btn-group">
				<a class="x_btn x_btn-inverse" href="<?php echo getUrl('', 'mid', 't1', 'act', 'dispMemberAdminInsert') ?>"><?php echo $__Context->lang->msg_new_member ?></a>
				<a href="#listManager" data-value="delete" class="modalAnchor _member x_btn">선택한 회원 삭제</a>
			</div>
		</caption>
		<thead>
			<tr>
				<?php if($__Context->usedIdentifiers&&count($__Context->usedIdentifiers))foreach($__Context->usedIdentifiers as $__Context->name=>$__Context->title){ ?><th scope="col" class="nowr"><?php echo $__Context->lang->email ?><p><?php echo $__Context->title ?></th><?php } ?>
				<th scope="col" class="nowr"><?php echo $__Context->lang->signup_date ?><p><?php echo $__Context->lang->last_login ?></a></th>
				<th scope="col" class="nowr"><?php echo $__Context->lang->member_group ?><p><?php echo $__Context->lang->inquiry ?>/<?php echo $__Context->lang->cmd_modify ?></th>
				<th scope="col">
		...
		<tbody>
			<?php if($__Context->member_list&&count($__Context->member_list))foreach($__Context->member_list as $__Context->no=>$__Context->member_info){ ?><tr>
				...
				<?php if($__Context->usedIdentifiers&&count($__Context->usedIdentifiers))foreach($__Context->usedIdentifiers as $__Context->name=>$__Context->title){ ?><td class="nowr">
					<?php echo getEncodeEmailAddress($__Context->member_info['email_address']) ?><p>
				...
				<td class="nowr" title="<?php echo zdate($__Context->member_info['regdate'], 'Y-m-d H:i:s') ?>"><?php echo zdate($__Context->member_info['regdate'], 'Y-m-d') ?><p><?php echo zdate($__Context->member_info['last_login'], 'Y-m-d') ?></td>
				<td><?php echo $__Context->member_info['group_list'] ?>&nbsp;<p><a href="<?php echo getUrl('', 'mid', 't1', 'act', 'dispMemberAdminInsert', 'member_srl', $__Context->member_info['member_srl']) ?>"><?php echo $__Context->lang->inquiry ?>/<?php echo $__Context->lang->cmd_modify ?></a></td>
...
<form action="./" method="get" class="search center x_input-append" ><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<input type="hidden" name="selected_group_srl" value="<?php echo $__Context->selected_group_srl ?>" />
	...
	<a class="x_btn" href="<?php echo getUrl('', 'mid', 't1', 'act', 'dispMemberAdminList', 'page', $__Context->page) ?>"><?php echo $__Context->lang->cmd_cancel ?></a>
</form>
